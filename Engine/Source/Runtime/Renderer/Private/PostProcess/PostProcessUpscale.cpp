...
void FRCPassPostProcessUpscale::Process(FRenderingCompositePassContext& Context)
{
	...
	if (View.StereoPass == eSSP_FULL || View.StereoPass == eSSP_LEFT_EYE)
	{
		...
		Context.RHICmdList.ClearColorTexture(DestRenderTarget.TargetableTexture, FLinearColor::Black, ExcludeRect); /*LaviniaADD seems like fc Clear was replaced by ClearColorTexture) ; */	}
